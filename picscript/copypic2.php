...
$list = file_list(".",'/(\.amr|\.jpeg|\.jpg|\.3gp|\.mp4|\.mov|\.mpg|\.avi|\.wav|\.aif|\.3gpp|\.aiff)$/i');
//var_dump($list);
echo "\n===================================================";
    ...
    if($dir_handle)
    {
        ...
        while(($file=readdir($dir_handle))!==false)
        {
            ...
            else
            {
                ...
                {
                    $fileext = $matches[0];
                    if(preg_match_all('/(20|19)([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])/i',$tmp,$grep_array)) {
                      echo $tmp."\n";
                      echo $file."\n";
                      $phototime=$grep_array[1][0].$grep_array[2][0]."-".$grep_array[3][0]."-".$grep_array[4][0]." ".$grep_array[5][0].":".$grep_array[6][0].":".$grep_array[7][0];
                                ...
                                while(1) {
                                    if(is_file($filename.$fileext)) {
                                        ...
                                        if($i>0) {
                                            ...
                                            if(!$found) {
                                                copy($tmp,$filename.$fileext);
                                                echo "copy to: ".$filename.$fileext."\n\n\n";
                                            } else {
                                                echo "duplicate.......... \n\n\n";
                                            }
                                        } else {
