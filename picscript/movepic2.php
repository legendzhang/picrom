        ...
        while (($file = readdir($dir_handle)) !== false) {
            ...
            if (is_dir($tmp)) {
                ...
                if ($pattern === "" || preg_match($pattern, $tmp, $matches)) {
                    ...
                    echo $file . "\n";
                    $phototime = "";
                    if (preg_match_all('/(VID)?[-._: ]?(20|19|V)([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])/i', $tmp, $grep_array)) {
                        if (strtoupper($grep_array[2][0]) == "V")
                            $grep_array[2][0] = "20";
                        $phototime = $grep_array[2][0] . $grep_array[3][0] . "-" . $grep_array[4][0] . "-" . $grep_array[5][0] . " " . $grep_array[6][0] . ":" . $grep_array[7][0] . ":" . $grep_array[8][0];
                        echo "使用文件名...\n";
                    } else if (preg_match_all('/(20|19)([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?([0-9][0-9])[-._: ]?[0-9]{3,5}/i', $tmp, $grep_array)) {
                        ...
                        if ($grep_array[2][0] == $filename_array[2][0] && $grep_array[3][0] == $filename_array[3][0] && $grep_array[4][0] == $filename_array[4][0]) {
                            ...
                            echo  "不移动：文件名 与 修改日期 不匹配 $filename\n";
                        }
                    } else {
                            echo  "不移动：文件名 不匹配\n";
                    }
